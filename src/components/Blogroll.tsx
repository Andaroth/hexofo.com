...
const Blogroll: FC = () => {
  ...
  useEffect(() => {
    if (!wpArticles.length) {
      fetch(WP_API + "media?per_page=30").then((res) => res.json()).then(medias => {
        setMedias(medias)
        fetch(WP_API + "posts").then((res) => res.json()).then(res => {
  ...
  return <Stack id="blog" bg="black" background="#070" py="4" style={{ marginTop: '0'}}>
    <Text as="h3" fontSize="xl" color="white" textAlign="center">Articles récents</Text>
      <Flex mt="4">
        <Flex w="100vw" overflowX="auto" justifyContent="left" pl="48px">
