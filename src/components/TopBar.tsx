...
const TopBar: FC<FlexProps> = (props) => {
  ...
  window.addEventListener('scroll', () => {
    setVisible(window.scrollY > (window.innerHeight - 48))
  })
  ...
  >
    <Flex w="100%">
      <Scroll to="top" smooth={true}>
        <Image cursor="pointer" src="./logo512.png" alt="logo" w="48px" h="48px" transition="all .5s ease" transform={ visible ? 'none' : 'translateY(-48px)'} />
      </Scroll>
      <Spacer />
      ...
      <Stack cursor="pointer" ml="4" _hover={{borderBottom: "2px solid #0F0", color: "#0F0"}}>
        <Scroll to="upcoming" smooth={true}>
          <Text lineHeight="48px"><span className="noMobile">Nos&nbsp;</span>Soirées</Text>
        </Scroll>
      </Stack>
      <Stack cursor="pointer" ml="4" _hover={{borderBottom: "2px solid #0F0", color: "#0F0"}}>
