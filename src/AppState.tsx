...
export const fetchBaseData = () => {
    ...
    if (!state.wpUsers.length) fetch(WP_API + "users?per_page=20").then((res) => res.json()).then(users => SET_USERS(users))

    if (!state.wpMedias.length || !state.wpArticles.length || !state.wpEvents.length) {
        fetch(WP_API + "media?per_page=60").then((res) => res.json()).then(medias => {
            SET_MEDIAS(medias)
